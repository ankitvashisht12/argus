...
/**
 * Map a raw CLI failure to an actionable, human message. Login and usage-limit
 * failures get a specific call to action; anything else passes through
 * unchanged. String-matching CLI output is brittle by nature, so the raw
 * message is preserved (appended) for diagnosis. Shared by the review
 * orchestrator and chat so both benefit.
 */
export function humanizeAgentError(message: string): string {
  const raw = oneLine(message);
  if (/not logged in|please run \/login|\/login|authentication_error|invalid api key|oauth/i.test(raw)) {
    return (
      'Claude Code is not logged in — run `claude` in a terminal and complete ' +
      `/login, then retry. (${raw.slice(0, 200)})`
    );
  }
  if (/rate.?limit|\b429\b|usage limit|overloaded|quota exceeded/i.test(raw)) {
    return (
      'Claude usage limit reached or service overloaded — try again in a bit. ' +
      `(${raw.slice(0, 200)})`
    );
  }
  return message;
...
export class ClaudeAgent {
  ...
  async chatStream(
    ...
    try {
      ...
      onDelta({
        type: 'error',
        text: oneLine(humanizeAgentError(message), 'Claude Code failed.'),
      });
      throw error;
    }
